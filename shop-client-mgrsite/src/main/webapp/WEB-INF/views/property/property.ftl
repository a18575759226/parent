...
<script type="text/javascript">
	...
	function zTreeOnClick(event, treeId, treeNode) {
		...
		//重新通过分类ID加载分类属性面板内容
		$("#catalogPropertyPanel").load("/getCatalogProperty.do?catalogId="+treeNode.id)
	...
	function showProperty(id){
		...
		$("#catalogPropertyValueForm").load("/getCatalogPropertyValue.do?catalogPropertyId="+id)
	...
	function deleteValue(obj,id){
		...
		if(id != 0){
			$.ajax({
				url:"/deleteCatalogPropertyValue.do?catalogPropertyId="+id,
				dataType:"json",
			})
		}
	}
	...
	function addCatalogProperty(propertyId){
		...
		$("#propertyFormContent").load("/toPropertySave.do?id="+propertyId+"&catalogId="+sNodes[0].id);
	...
	function deleteCatalogProperty(id,catalogId){
		$.messager.confirm("提示", "确定删除吗？", function() {
			...
			$("#catalogPropertyPanel").load("/deleteCatalogProperty.do?id="+id+"&catalogId="+catalogId);
		});
		...
		<div class="modal fade" id="propertyValueModal" tabindex="-1" role="dialog"
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<!-- 提交添加分类属性值的表单 -->
					<form action="/addCatalogPropertyValue.do" method="post" id="catalogPropertyValueForm">
